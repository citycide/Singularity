...
module.exports.rekt = event => {
    ...
    if (event.subcommand === 'add') {
        ...
        $.db.set('rekt', { value: `☐ Not rekt ☑ ${event.subArgString}` });

        const res = $.db.getRow('rekt', { value: `☐ Not rekt ☑ ${event.subArgString}` });
    ...
    if (event.subcommand === 'edit') {
        ...
        if (!$.db.exists('rekt', { id: event.subArgs[0] })) {
...
function initResponses() {
    ...
    for (let response of defaults) {
        $.db.set('rekt', { value: response });
    }
...
(() => {
    ...
    $.db.addTable('rekt', true);

    if (!$.db.countRows('rekt')) initResponses();
})();
