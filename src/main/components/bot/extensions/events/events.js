...
export async function alerts (e, $) {
  ...
  if ($.is('follow')) {
    if (param1 === 'enable') {
      ...
      $.say(e.sender, weave.get('bot:settings:alerts-follows:disabled:success'))
    } else {
      const status = await $.settings.get('followAlerts', true)
        ? weave.get('common-words:enabled')
        : weave.get('common-words:disabled')
      $.say(e.sender, weave.get('bot:modules:events:alerts:follow:usage', status))
    }
  ...
  if ($.is('host')) {
    if (param1 === 'enable') {
      ...
      $.say(e.sender, weave.get('bot:settings:alerts-hosts:disabled:success'))
    } else {
      const status = await $.settings.get('hostAlerts', true)
        ? weave.get('common-words:enabled')
        : weave.get('common-words:disabled')
      $.say(e.sender, weave.get('bot:modules:events:alerts:host:usage', status))
    }
  ...
  if ($.is('sub')) {
    if (param1 === 'enable') {
      ...
      $.say(e.sender, weave.get('bot:settings:alerts-subs:disabled:success'))
    } else {
      const status = await $.settings.get('subAlerts', false)
        ? weave.get('common-words:enabled')
        : weave.get('common-words:disabled')
      $.say(e.sender, weave.get('bot:modules:events:alerts:sub:usage', status))
    }
  ...
  if ($.is('tip')) {
    if (param1 === 'enable') {
      ...
      $.say(e.sender, weave.get('bot:settings:alerts-tips:disabled:success'))
    } else {
      const status = await $.settings.get('tipAlerts', false)
        ? weave.get('common-words:enabled')
        : weave.get('common-words:disabled')
      $.say(e.sender, weave.get('bot:modules:events:alerts:tip:usage', status))
    }
  ...
  if ($.is('settings')) {
    ...
    $.say(e.sender, weave.get('bot:modules:events:alerts:settings', ...cfg))

    return
  }

  $.say(e.sender, weave.get('bot:modules:events:alerts:usage'))
}
